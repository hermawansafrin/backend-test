                ...
                <div class="col-xl-12">
                    <div class="card shadow-sm">
                        ...
                        <div class="card-body">
                            ...
                            <form action="{{ route($ROUTE_PATH.'update', $params) }}" method="POST" id="transactionForm">
                                @csrf
                                @method('PUT')
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <select class="form-control select2" id="customer_id" name="customer_id" required>
                                        ...
                                        @foreach($customers as $customer)
                                            <option value="{{ $customer['id'] }}" {{ $editedData['customer_id'] == $customer['id'] ? 'selected' : '' }}>{{ $customer['name'] }}</option>
                                        @endforeach
                                    </select>
                                    @error('customer_id')
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <input type="number" class="form-control" id="discount_percentage" name="discount_percentage" min="0" max="100" value="{{ $editedData['discount_percentage'] ?? 0 }}" required>
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <textarea class="form-control" id="note" name="note" rows="3" maxlength="500" placeholder="{{ __('Enter note here (optional)') }}">{{ $editedData['note'] ?? '' }}</textarea>
                                    @error('note')
                                        <span class="text-danger ml-2" style="font-size:14px;">
                                            {{ $message }}
                                        </span>
                                ...
                                <div class="table-responsive mt-4">
                                    <table class="table table-bordered" id="productTable">
                                        ...
                                        <tbody>
                                            @foreach($editedData['transaction_items'] as $item)
                                                <tr data-product-id="{{ $item['product_id'] }}">
                                                    <td>{{ $item['product']['name'] }}</td>
                                                    <td>{{ $products[$item['product_id']]['stock'] }}</td>
                                                    <td>{{ $item['price'] }}</td>
                                                    <td>
                                                        <input type="number" class="form-control qty-input"
                                                            min="1" max="{{ $products[$item['product_id']]['stock'] }}"
                                                            value="{{ $item['qty'] }}"
                                                            data-price="{{ $item['price'] }}"
                                                            data-stock="{{ $products[$item['product_id']]['stock'] }}">
                                                    </td>
                                                    <td class="item-total">{{ $item['total_amount'] }}</td>
                                                    <td>
                                        ...
                                        <tfoot>
                                            <tr>
                                                ...
                                                <td colspan="2"><strong id="totalWithoutDiscount">{{ $editedData['total_without_discount'] }}</strong></td>
                                            </tr>
                                            <tr>
                                                <td colspan="4" class="text-right"><strong>Total Discount:</strong></td>
                                                <td colspan="2"><strong id="totalDiscount">{{ $editedData['total_discount'] }}</strong></td>
                                            </tr>
                                            <tr>
                                                <td colspan="4" class="text-right"><strong>Amount/Grand Total:</strong></td>
                                                <td colspan="2"><strong id="grandTotal">{{ $editedData['total_amount'] }}</strong></td>
                                            </tr>
                                        </tfoot>
                                    </table>
                                </div>
    ...
    <script>
        $(function(){
            ...
            $('#productTable tbody tr').each(function() {
                ...
                const stock = $(this).find('.qty-input').data('stock');
                const price = $(this).find('.qty-input').data('price');
            ...
            // Initialize discount percentage from editedData
            $('#discount_percentage').val('{{ $editedData['discount_percentage'] }}');
